from Crypto.PublicKey import RSA

# La chiave pubblica fornita nel testo
ssh_key = "ssh-rsa  bschneier@facts"

# Importiamo la chiave
# Nota: RSA.import_key gestisce nativamente il formato della chiave pubblica SSH
key = RSA.import_key(ssh_key)

# Estraiamo il modulo n
modulus_n = key.n

print(f"Il modulo n in formato decimale è:\n{modulus_n}")