...
```bash
docker run -ti --net host --volume "$PWD":/vapps -v /:/altro dozenapps/fva:20250925 /bin/bash -c 'cd /vapps; /bin/bash -i'
```
...
- oppure clean-all.sh
- per eliminare anche tutti i .sh
sudo rm -rf clean-all.sh clean-start.sh simply-start.sh stop-blockchain.sh clean-blockchain.sh compile-and-deploy-chaincode.sh stop-blockchain-clean-chaincode.sh core.yaml 

# Per interrogare e operare con la blockchain
