...
def welcome ():
    links:dict={
        "lista_veicolo":url_for("vheicles"),
        "veicolo_specifico":url_for("get_plate",plate_id="HA014AS"),
        "estimate":url_for("device",plate_id='HA014AS', factor=2.0)
    }
    return jsonify({
        "messaggio":"Welcome to Rent Service API","links":links
    })
...
def get_plate(plate_id:str):
    veicolo=fleet_manager.get(plate_id)
    if not veicolo:
        return jsonify({"message":"Errore, il veicolo non esiste"}),404
    else:
        return jsonify(veicolo.info()),200
...
def device(plate_id:str,factor:float):
    ...
    if not veicolo:
        ...
        info:dict=veicolo.info()
        info['tempo_attesa']=veicolo.estimated_prep_time(factor)
