...
Tale funzione deve utilizzare un ciclo per iterare la lista e, per ogni dimensione non compressa, deve calcolare la dimensione compressa di tale file espressa 
come float (ovvero deve calcolare l' 80% della dimensione non compressa), calcolare il numero di blocchi (arrotondato al numero intero più vicino) da 
512 byte necessari per la memorizzazione, al fine di determinare se il file compresso può essere salvato nello spazio rimanente nel supporto di memorizzazione o meno.
...
Il ciclo continua finché non viene riscontrato un file della lista la cui dimensione compressa occupa un numero di blocchi più grande di quelli rimasti
 disponibili sul supporto di memorizzazione. In tal caso, la funzione deve avvisare l'utente che lo spazio disponibile sul supporto di memorizzazione 
 non è sufficiente per salvare il file. 
...
def memorizza_file(files: list[int]) -> None:
    ...
    dimensione_blocco = 512  # Ogni blocco ha 512 byte

    # Ciclo per ogni file nella lista
    for file in files:
        # Calcolo della dimensione compressa (80% della dimensione originale)
        dimensione_compressa = file * 0.80

        # Calcolo del numero di blocchi necessari per memorizzare il file compresso
        blocchi_necessari = math.ceil(dimensione_compressa / dimensione_blocco)

        # Verifica se ci sono abbastanza blocchi disponibili
        if blocchi_necessari <= spazio_totale_blocchi:
            # Memorizza il file
            spazio_totale_blocchi -= blocchi_necessari
            print(f"File di {file} byte compresso in {dimensione_compressa} byte e memorizzato. "
                  f"Blocchi usati: {blocchi_necessari}. Blocchi rimanenti: {spazio_totale_blocchi}.")
        else:
            # Spazio insufficiente per memorizzare il file
            print(f"Non è possibile memorizzare il file di {file} byte. Spazio insufficiente.")
            break  # Esce dal ciclo quando non è possibile memorizzare un file
