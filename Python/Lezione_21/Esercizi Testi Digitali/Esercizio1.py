...
class Documento:
    ...
    def set_testo(self,text:str)-> None:
        self.testo=text
...
class Email(Documento):
    ...
    def __init__(self, testo, mittente:str, destinatario:str, titolo_mess:str):
        super().__init__(testo)
        

        self.set_mittente(mittente)
        self.set_destinatario(destinatario)
        self.set_titolo_mex(titolo_mess)
    

    def set_mittente(self, mittente)->None:
        self.mittente=mittente
    
    def set_destinatario(self, destinatario) ->None:
        self.destinatario=destinatario
    
    def set_titolo_mex(self,titolo)->None:
        self.titolo_mess=titolo
    ...
    def get_titolo_mex(self) ->str:
        return self.titolo_mess


    def getText(self):

        return f"Da: {self.mittente}@example.com, A: {self.destinatario}@example.com \n\
Titolo: {self.titolo_mess}\n\
Messaggio: {self.testo}"

    def writeToFile(self)->None:
        with open ("/home/its/ITS/Python/Lezione_21/Esercizi Testi Digitali/documet.txt", "w") as file:
...
class File(Documento):
    def __init__(self, nomePercorso: str):
        ...
        super().__init__(self.testo)

    def leggiTestoDaFile(self) -> None:
        percorso_completo = f"{self.nomePercorso}/document.txt"
        with open(percorso_completo, "r") as file:
            self.testo = file.read().strip()
...
email = Email(
    ...
    mittente="alice",
    destinatario="bob",
    titolo_mess="Incontro"
)
print(email.getText())
email.writeToFile()

# File
file = File("/home/its/ITS/Python/Lezione_21/Esercizi Testi Digitali")
print(file.getText())
