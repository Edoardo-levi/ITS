...
def serieNomi():
    ...
    while True:
        ...
        if len(nome)>20 or nome == "":
            ...
            print("Nome troppo lungo o non valido")

            nome:str=input("Inserisci un nome:\n").strip().title()
        else:
            if nome in listaNomi:
    ...
    for index in listaNomi:
        if len(index) > len(massimoNome):
            massimoNome = index
            massimo = len(index)
    
    print (f"Sono stati inseriti {len(listaNomi)} nomi")
    print (f"Il nome più lungo inserito è: {massimoNome} con lunghezza di {massimo} caratteri")
